...
class FlexContextSchema(Schema):
    ...
    consumption_price_sensor_per_device = fields.Dict(
        keys=SensorIdField(), values=SensorIdField(), data_key="consumption-price-sensor-per-device"
    )
    production_price_sensor_per_device = fields.Dict(
        keys=SensorIdField(), values=SensorIdField(), data_key="production-price-sensor-per-device"
    )
